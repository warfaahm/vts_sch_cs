...
import ProviderAccountListForm from "@/components/accounts/ProviderAccountListForm";


export default function ProviderAccountList()
{
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://127.0.0.1:8000/api/staff/provider/', {
                headers: {
    ...
    const handleDeleteClick = async (id) => {
        try {
            const response = await axios.delete(`http://127.0.0.1:8000/api/staff/provider/${id}`, {
                headers: {
                    'Authorization': `Bearer ${token1}`
                }
            });
