...
import AddDependentRecord from "@/components/record/AddDependentRecord";


export default function SearchResultDependent(props)
{
    ...
    const fetchData = async () => {
        try {
            const response = await axios.post('http://127.0.0.1:8000/api/staff/search_record2/', {
                last_name: name,
        ...
        <div>
            <div className='bg-blue-50 px-4 py-6 rounded-lg mt-4'>
                <div className='border-b-2 border-gray-300 mb-2'>
                    <div className='flex'>
                        <div className='m-2'>
                            ...
                            <h1 className='dialog2'>{data?.patient?.first_name+ '  '+data?.patient?.last_name}</h1>
                        </div>
                        <div className='m-2'>
                            <span className='dialog1'>Email</span>
                            <h1 className='dialog2'>{data?.parent?.user?.email}</h1>
                        </div>
                        <div className='m-2'>
                    ...
                    <div className='flex'>
                        ...
                        <div className='m-2'>
                            <span className='dialog1'>Birth Certificate No</span>
                            <h1 className='dialog2'>{data?.patient?.birth_cert_no}</h1>
                        </div>
                        <div className='m-2'>
                    ...
                    <div className='flex'>
                        <div className='m-2'>
                            <span className='dialog1'>Parent Name</span>
                            <h1 className='dialog2'>{data?.parent?.user?.first_name+ '  '+data?.parent?.user?.last_name}</h1>
                        </div>
                        <div className='m-2'>
                            <span className='dialog1'>National ID</span>
                            <h1 className='dialog2'>{data?.parent?.nat_id_no}</h1>
                        </div>
                        <div className='m-2'>
                            <span className='dialog1'>Relationship</span>
                            <h1 className='dialog2'>{data?.patient?.relationship}</h1>
                        </div>
                    </div>
                </div>
                ...
                <Popup
                    ...
                    title='Record Info'
                >
                    <AddDependentRecord id = {data?.patient?.id} />
                </Popup>
            </div>
        </div>
    )
