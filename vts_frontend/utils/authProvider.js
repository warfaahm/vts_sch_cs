const isLoggedIn = async () => {
    try {
        ...
        const response = await fetch('http://127.0.0.1:8000/api/staff/auth/check', {
            // Replace with your Laravel API endpoint for checking authentication
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('staffToken')}` // Send token in Authorization header
            },
        });
